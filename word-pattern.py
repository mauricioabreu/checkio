def check_command(pattern, command):
    bin_pattern = str('{0:0b}'.format(pattern))
    bin_command = ''.join(map(lambda x: '1' if x.isalpha() else '0', command))
    bin_pattern = bin_pattern.rjust(len(bin_command), '0')
    
    return bin_pattern == bin_command
